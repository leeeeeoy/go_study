...
func (cq *CommentQuery) QueryBoard() *BoardQuery {
	query := (&BoardClient{config: cq.config}).Query()
...
func (cq *CommentQuery) QueryCommentLike() *CommentLikeQuery {
	query := (&CommentLikeClient{config: cq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := cq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (cq *CommentQuery) QueryCommentMention() *CommentMentionQuery {
	query := (&CommentMentionClient{config: cq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := cq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (cq *CommentQuery) WithBoard(opts ...func(*BoardQuery)) *CommentQuery {
	query := (&BoardClient{config: cq.config}).Query()
...
func (cq *CommentQuery) WithCommentLike(opts ...func(*CommentLikeQuery)) *CommentQuery {
	query := (&CommentLikeClient{config: cq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	cq.withCommentLike = query
	return cq
}
...
func (cq *CommentQuery) WithCommentMention(opts ...func(*CommentMentionQuery)) *CommentQuery {
	query := (&CommentMentionClient{config: cq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (cq *CommentQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Comment, error) {
	...
	if query := cq.withBoard; query != nil {
		if err := cq.loadBoard(ctx, query, nodes, nil,
			func(n *Comment, e *Board) { n.Edges.Board = e }); err != nil {
			return nil, err
		}
	}
	...
	if query := cq.withCommentLike; query != nil {
		if err := cq.loadCommentLike(ctx, query, nodes,
			func(n *Comment) { n.Edges.CommentLike = []*CommentLike{} },
			func(n *Comment, e *CommentLike) { n.Edges.CommentLike = append(n.Edges.CommentLike, e) }); err != nil {
			return nil, err
		}
	}
	if query := cq.withCommentMention; query != nil {
...
func (cq *CommentQuery) loadBoard(ctx context.Context, query *BoardQuery, nodes []*Comment, init func(*Comment), assign func(*Comment, *Board)) error {
...
func (cq *CommentQuery) loadCommentLike(ctx context.Context, query *CommentLikeQuery, nodes []*Comment, init func(*Comment), assign func(*Comment, *CommentLike)) error {
	...
	query.Where(predicate.CommentLike(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(comment.CommentLikeColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
...
func (cq *CommentQuery) loadCommentMention(ctx context.Context, query *CommentMentionQuery, nodes []*Comment, init func(*Comment), assign func(*Comment, *CommentMention)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int]*Comment)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(commentmention.FieldCommentID)
	}
...
func (cq *CommentQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := cq.ctx.Fields; len(fields) > 0 {
		...
		if cq.withBoard != nil {
			_spec.Node.AddColumnOnce(comment.FieldBoardID)
		}
		if cq.withUser != nil {
