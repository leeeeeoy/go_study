...
func (cq *CategoryQuery) QueryTopics() *TopicQuery {
...
func (cq *CategoryQuery) WithTopics(opts ...func(*TopicQuery)) *CategoryQuery {
	query := (&TopicClient{config: cq.config}).Query()
...
func (cq *CategoryQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Category, error) {
	...
	if query := cq.withTopics; query != nil {
		if err := cq.loadTopics(ctx, query, nodes,
			func(n *Category) { n.Edges.Topics = []*Topic{} },
			func(n *Category, e *Topic) { n.Edges.Topics = append(n.Edges.Topics, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (cq *CategoryQuery) loadTopics(ctx context.Context, query *TopicQuery, nodes []*Category, init func(*Category), assign func(*Category, *Topic)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(topic.FieldCategoryID)
	}
	query.Where(predicate.Topic(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(category.TopicsColumn), fks...))
	}))
