...
func (tq *TopicQuery) QueryBoards() *BoardQuery {
...
func (tq *TopicQuery) QueryCategory() *CategoryQuery {
...
func (tq *TopicQuery) Clone() *TopicQuery {
	...
	return &TopicQuery{
		...
		withBoards:   tq.withBoards.Clone(),
...
func (tq *TopicQuery) WithBoards(opts ...func(*BoardQuery)) *TopicQuery {
	query := (&BoardClient{config: tq.config}).Query()
...
func (tq *TopicQuery) WithCategory(opts ...func(*CategoryQuery)) *TopicQuery {
	query := (&CategoryClient{config: tq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (tq *TopicQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Topic, error) {
	...
	if query := tq.withBoards; query != nil {
		if err := tq.loadBoards(ctx, query, nodes,
			func(n *Topic) { n.Edges.Boards = []*Board{} },
			func(n *Topic, e *Board) { n.Edges.Boards = append(n.Edges.Boards, e) }); err != nil {
			return nil, err
		}
	}
	if query := tq.withCategory; query != nil {
		if err := tq.loadCategory(ctx, query, nodes, nil,
			func(n *Topic, e *Category) { n.Edges.Category = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (tq *TopicQuery) loadBoards(ctx context.Context, query *BoardQuery, nodes []*Topic, init func(*Topic), assign func(*Topic, *Board)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(board.FieldTopicID)
	}
	query.Where(predicate.Board(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(topic.BoardsColumn), fks...))
	}))
...
func (tq *TopicQuery) loadCategory(ctx context.Context, query *CategoryQuery, nodes []*Topic, init func(*Topic), assign func(*Topic, *Category)) error {
...
func (tq *TopicQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := tq.ctx.Fields; len(fields) > 0 {
		...
		if tq.withCategory != nil {
			_spec.Node.AddColumnOnce(topic.FieldCategoryID)
		}
	}
